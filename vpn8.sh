#!/bin/bash
# ******************************************
# Program: anonymousVpn8
# Website: anonymousVpn8.tk
# Developer: anonymousVpn8
...
wget -O /etc/nginx/nginx.conf "https://raw.githubusercontent.com/Autoscriptsabahan/isox/master/nginx.conf"
mkdir -p /home/vps/public_html
echo "<pre>Modified by Blackapache9</pre>" > /home/vps/public_html/index.html
echo "<?php phpinfo(); ?>" > /home/vps/public_html/info.php
wget -O /etc/nginx/conf.d/vps.conf "https://raw.githubusercontent.com/Autoscriptsabahan/isox/master/vps.conf"
...
if [[ $vps = "zvur" ]]; then
	...
	echo "anonymousVpn8/" | tee -a log-install.txt
else
	echo "ALL SUPPORTED BY anonymousVpn8 VPS" | tee -a log-install.txt
	echo "https://www.facebook.com/aneka.vps.us" | tee -a log-install.txt
fi

echo "DEVELOPED BY anonymousVpn8" | tee -a log-install.txt
echo "Credit to Yurissh OpenSource, Hariyanto Dwi Semangat, Nami Kulo Barkah, Micky Maximus" | tee -a log-install.txt
echo "Thanks to Original Creator anonymousVpn8" | tee -a log-install.txt
echo "" | tee -a log-install.txt
